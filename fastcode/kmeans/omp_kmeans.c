                    ...
                    float *coord2)   /* [numdims] */
{
    ...
    float ans=0.0;
     ...
     int rounded_i = numdims;
     {
       int misalignment_c1 = ((size_t)coord1) % 16;
       if (misalignment_c1 == ((size_t)coord2) % 16) {
         if (misalignment_c1 == 12) {
           rounded_i = 1;
         } else if (misalignment_c1 == 8) {
           rounded_i = 2;
         } else if (misalignment_c1 == 4) {
           rounded_i = 3;
         } else if (misalignment_c1 == 0) {
           rounded_i = 0;
         }
       }
     }
     if (rounded_i > numdims)
       rounded_i = numdims;
    
     for (; i<rounded_i; i++)
         ans += coord1[i]-coord2[i] * (coord1[i]-coord2[i]);
    
    __m128 c1a, c2a, c1b, c2b;
    __m128 result;
    result = _mm_set1_ps(0.0f);

    for (; i+8 <= numdims;i+=8) {
      c1a = _mm_load_ps(coord1 + i);
      c2a = _mm_load_ps(coord2 + i);
      c1a = _mm_sub_ps(c1a, c2a);
      c1a = _mm_mul_ps(c1a, c1a);
      c1b = _mm_load_ps(coord1 + i + 4);
      c2b = _mm_load_ps(coord2 + i + 4);
      c1b = _mm_sub_ps(c1b, c2b);
      c1b = _mm_mul_ps(c1b, c1b);
      result = _mm_add_ps(result, _mm_add_ps(c1a, c1b));
    }
